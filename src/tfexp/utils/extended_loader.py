    ...
    class ExtendedLoader(yaml.Loader):
        ...
        def __init__(self, stream: IO) -> None:
            ...
            self.add_implicit_resolver(
                u"!store", self.set_global_variable_pattern, None
            )

            self.add_implicit_resolver(u"!load", self.get_global_variable_pattern, None)
        ...
        def _include_constructor(self, loader, node):
            path = os.path.abspath(
                os.path.join(loader._root, self.construct_scalar(node))
            )
